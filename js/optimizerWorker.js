...
async function ensureWasm() {
    ...
    try {
        const { default: init, WmoEngine } = await import("./wasm/wmo_engine.js");
        await init();
        wasmEngine = new WmoEngine(BigInt(Date.now()));
...
self.onmessage = async function (e) {
    try {
        ...
        const config = buildConfig({
            engineerLevel, scarabLevel, globalRarityLevels,
            riftRank, artifactArray, maxMission, heroScoring, ownedHeroes,
        });
...
function buildConfig({ engineerLevel, scarabLevel, globalRarityLevels, riftRank,
    artifactArray, maxMission, heroScoring, ownedHeroes }) {
...
function reconstructResult(wasmResult, ownedMachines, ownedHeroes) {
    ...
    const formation = (wasmResult.formation ?? []).map(r => {
        const base = machineMap.get(r.id);
        if (!base) return null;

        const crew = (r.assigned_hero_ids ?? [])
...
function isDecimalLike(obj) {
    return obj !== null
        && typeof obj === "object"
        && typeof obj.toExponential === "function"
        && "sign" in obj && "layer" in obj && "mag" in obj;
}
